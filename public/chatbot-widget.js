...
window.RAG_CHATBOT_CONFIG = {
  ...
  name: "Kevin's AI Assistant",
  primaryColor: "#1f7a5c",
  secondaryColor: "#238a68",
  greeting:
    "Hi! I'm Kevin's AI assistant. Ask me anything about his background, experience, and work.",
  bubbleLabel: "Ask Kevin's AI",
  suggestedQuestions: [
    "What's Kevin's background?",
    "What has he led or delivered?",
...
(function () {
  ...
  if (!CONFIG.apiEndpoint) {
    console.error(
      "[RAG Chatbot] apiEndpoint is required in RAG_CHATBOT_CONFIG",
    );
    return;
  }
  ...
  const Widget = (function () {
    ...
    function addMessage(sender, text) {
      setState({
        messages: [...state.messages, { sender, text, ts: Date.now() }],
      });
        ...
        state.messages.length > 0
          ? state.messages
              .map((m) => {
                const cls =
                  m.sender === "user"
                    ? "rag-chatbot-msg-user"
                    : m.sender === "error"
                      ? "rag-chatbot-msg-error"
                      : "rag-chatbot-msg-bot";
